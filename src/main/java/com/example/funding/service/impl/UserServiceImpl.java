...
public class UserServiceImpl implements UserService {
    ...
    public ResponseEntity<ResponseDto<String>> signUp(SignUpRequestDto dto) {
        ...
        if (existingUser != null) {
            return ResponseEntity.badRequest().body(ResponseDto.fail("이미 사용중인 이메일입니다."));
        }
        ...
        User user = User.builder().email(dto.getEmail()).password(dto.getPassword()).nickname(dto.getNickname())
                .build();
        userDao.signUp(user);
        return ResponseEntity.ok(ResponseDto.success("회원가입 성공", dto.getEmail()));
    }
    ...
    public ResponseEntity<ResponseDto<String>> signIn(SignInRequestDto dto) {
        ...
        if (user == null) {
            return ResponseEntity.badRequest().body(ResponseDto.fail("존재하지 않는 이메일입니다."));
        }
        String encodedPassword = passwordEncoder.encode(dto.getPassword());
        if (passwordEncoder.matches(encodedPassword, user.getPassword())) {
            return ResponseEntity.badRequest().body(ResponseDto.fail("비밀번호가 일치하지 않습니다."));
        }
