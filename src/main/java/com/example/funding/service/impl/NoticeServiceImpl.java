...
public class NoticeServiceImpl implements NoticeService {
    ...
    @Override
    public ResponseEntity<ResponseDto<Notice>> getNoticeDetail(Long noticeId) {
    ...
    public ResponseEntity<ResponseDto<Notice>> add(Notice item) {
        ...
        if (noticeAdd == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseDto.fail(409,"공지사항 등록 불가"));
        }
    ...
    public ResponseEntity<ResponseDto<Notice>> update(Notice item) {
        ...
        if (noticeUpdate == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseDto.fail(409,"공지사항 수정 불가"));
        }
        return ResponseEntity.status(HttpStatus.OK).body(ResponseDto.success(200,"공지사항 수정 성공", noticeUpdate));
    }

    @Override
    public ResponseEntity<ResponseDto<Notice>> delete(Long noticeId) {
        Notice noticeDelete = noticeMapper.delete(noticeId);
        if (noticeDelete == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ResponseDto.fail(409,"공지사항 삭제 불가"));
        }
